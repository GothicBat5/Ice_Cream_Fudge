...
#define P(name, number_of_args, result_size, ...) \
  ObjectPair Runtime_##name(int args_length, Address* args_object, \ Isolate* isolate);
...
{
  IntrinsicFunctionIdentifier(const unsigned char* data, const int length): data_(data), length_(length) {}

  static bool Match(void* key1, void* key2) 
{
    const IntrinsicFunctionIdentifier* lhs = static_cast<IntrinsicFunctionIdentifier*>(key1);
    const IntrinsicFunctionIdentifier* rhs = static_cast<IntrinsicFunctionIdentifier*>(key2);
  ...
  uint32_t Hash() 
{
    return StringHasher::HashSequentialString<uint8_t>(data_, length_, HashSeed::Default());
  }
...
{
  base::CustomMatcherHashMap* function_name_map = new base::CustomMatcherHashMap(IntrinsicFunctionIdentifier::Match);
  
  for (size_t i = 0; i < arraysize(kIntrinsicFunctions); ++i) 
  {
    const Runtime::Function* function = &kIntrinsicFunctions[i];
    IntrinsicFunctionIdentifier* identifier = new IntrinsicFunctionIdentifier(reinterpret_cast<const unsigned char*>(function->name),
    static_cast<int>(strlen(function->name)));
    base::HashMap::Entry* entry = function_name_map->InsertNew(identifier, identifier->Hash());
...
bool Runtime::IsEnabledForFuzzing(FunctionId id) 
{
  ...
  bool is_differential_fuzzing = v8_flags.allow_natives_for_differential_fuzzing;
  ...
  switch (id) 
  {
    ...
    case Runtime::kInstallBytecode:
      return (v8_flags.sandbox_testing || v8_flags.sandbox_fuzzing) && v8_flags.verify_bytecode_full;
    case Runtime::kIsSmi:
...
const Runtime::Function* Runtime::FunctionForName(const unsigned char* name, int length) 
{
  base::CallOnce(&initialize_function_name_map_once, &InitializeIntrinsicFunctionNames);
  IntrinsicFunctionIdentifier identifier(name, length);
  base::HashMap::Entry* entry = kRuntimeFunctionNameMap->Lookup(&identifier, identifier.Hash());
...
const Runtime::Function* Runtime::RuntimeFunctionTable(Isolate* isolate) 
{
  ...
  if (!isolate->runtime_state()->redirected_intrinsic_functions()) 
  {
    ...
    memcpy(redirected_functions, kIntrinsicFunctions, sizeof(kIntrinsicFunctions));
    
    for (size_t i = 0; i < function_count; i++) 
    {
      ExternalReference redirected_entry = ExternalReference::Create(static_cast<Runtime::FunctionId>(i));
      redirected_functions[i].entry = redirected_entry.address();
    }
    
    isolate->runtime_state()->set_redirected_intrinsic_functions(redirected_functions);
  }
