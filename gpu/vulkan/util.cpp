...
{
  auto disable_patterns = base::SplitString(block_list, "|", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
...
int GetEMUIVersion() 
{
  ...
  int version = 0;
  if (UNSAFE_TODO(sscanf(base::android::android_info::version_incremental().c_str(), "%d.", &version)) != 1) 
  {
    return -1;
  }
...
bool IsBlockedByBuildInfo() 
{
  ...
  const char* kBlockListByBoard = "RM67*|RM68*|k68*|mt6*|oppo67*|oppo68*|QM215|rk30sdk";

  if (IsDeviceBlocked(base::android::android_info::hardware(), kBlockListByHardware)) 
  {
    return true;
  }
  if (IsDeviceBlocked(base::android::android_info::brand(), kBlockListByBrand)) 
  {
    return true;
  }
  if (IsDeviceBlocked(base::android::android_info::device(), kBlockListByDevice)) 
  {
    return true;
  }
  if (IsDeviceBlocked(base::android::android_info::board(), kBlockListByBoard)) 
  {
    return true;
  }
...
VulkanPhysicalDeviceProperties::VulkanPhysicalDeviceProperties(const VkPhysicalDeviceProperties& properties)
...
bool SubmitSignalVkSemaphores(VkQueue vk_queue, const base::span<VkSemaphore>& vk_semaphores, VkFence vk_fence) 
{
  ...
  return vkQueueSubmit(vk_queue, submit_count, &submit_info, vk_fence) == VK_SUCCESS;
}

bool SubmitSignalVkSemaphore(VkQueue vk_queue, VkSemaphore vk_semaphore, VkFence vk_fence) 
{
  return SubmitSignalVkSemaphores(vk_queue, UNSAFE_TODO(base::span<VkSemaphore>(&vk_semaphore, 1u)), vk_fence);
}

bool SubmitWaitVkSemaphores(VkQueue vk_queue, const base::span<VkSemaphore>& vk_semaphores, VkFence vk_fence) 
{
  DCHECK(!vk_semaphores.empty());
  std::vector<VkPipelineStageFlags> semaphore_stages(vk_semaphores.size());
  std::fill(semaphore_stages.begin(), semaphore_stages.end(), VK_PIPELINE_STAGE_ALL_COMMANDS_BIT);
  VkSubmitInfo submit_info = {VK_STRUCTURE_TYPE_SUBMIT_INFO};
...
bool SubmitWaitVkSemaphore(VkQueue vk_queue, VkSemaphore vk_semaphore, VkFence vk_fence) 
{
  return SubmitWaitVkSemaphores(vk_queue, UNSAFE_TODO(base::span<VkSemaphore>(&vk_semaphore, 1u)), vk_fence);
}

VkSemaphore CreateExternalVkSemaphore(VkDevice vk_device, VkExternalSemaphoreHandleTypeFlags handle_types) 
{
  ...
  VkResult result = vkCreateSemaphore(vk_device, &sem_info, nullptr, &semaphore);
...
std::string VkVersionToString(uint32_t version) {
  return base::StringPrintf("%u.%u.%u", VK_VERSION_MAJOR(version), VK_VERSION_MINOR(version), VK_VERSION_PATCH(version));
}

VkResult CreateGraphicsPipelinesHook(VkDevice device, VkPipelineCache pipelineCache, uint32_t createInfoCount,
...
VkResult VulkanQueueSubmitHook(VkQueue queue, uint32_t submitCount, const VkSubmitInfo* pSubmits, VkFence fence) 
{
  absl::Cleanup uma_runner = [start_time = base::TimeTicks::Now()] 
  {
    base::TimeDelta elapsed = base::TimeTicks::Now() - start_time;
...
VkResult VulkanQueuePresentKHRHook(VkQueue queue, const VkPresentInfoKHR* pPresentInfo) 

  TRACE_EVENT0("gpu", "VulkanQueuePresentKHRHook");
  return vkQueuePresentKHR(queue, pPresentInfo);
}

bool CheckVulkanCompatibilities(const VulkanPhysicalDeviceProperties& device_properties, const GPUInfo& gpu_info) 
{
#if !BUILDFLAG(IS_ANDROID)
#if BUILDFLAG(IS_WIN)
  ...
  if (!base::FeatureList::IsEnabled(features::kVulkanFromANGLE)) 
  {
    ...
    {
        DLOG(ERROR) << kMemoryObjectExtension << " or " << kSemaphoreExtension << " is not supported.";
        return false;
    }
  }
  ...
  if (device_properties.vendor_id == kVendorARM) 
  {
    ...
    std::vector<const char*> slow_gpus = {"2??", "3??", "4??", "T???", "G31", "G51", "G52"};
    ...
    if (device_name == "G57" && device_properties.driver_version < VK_MAKE_VERSION(41, 0, 0)) 
    {
      return false;
    }
  ...
  if (device_properties.vendor_id == kVendorQualcomm) 
  {
    return device_properties.device_name == std::string_view("Adreno (TM) 630") &&
           !IsDeviceBlocked(gpu_info.gpu.driver_version, "324.0|331.0|334.0|378.0|415.0|420.0|444.0");
  }

  if (device_properties.vendor_id == kVendorImagination) 
  {
    return RE2::FullMatch(device_properties.device_name, "PowerVR ([CDE]-Series)? [CDE]X.*");
  }
...
bool IsVkExternalSemaphoreHandleTypeSupported(VulkanDeviceQueue* device_queue,
    VkExternalSemaphoreHandleTypeFlagBits handle_type) 
{
  ...
  vkGetPhysicalDeviceExternalSemaphoreProperties(physical_device, &semaphore_info, &semaphore_properties);
...
VkResult QueryVkExternalMemoryProperties(VkPhysicalDevice physical_device,
    VkFormat format, VkImageType type,
    VkImageTiling tiling, VkImageUsageFlags usage,
    VkImageCreateFlags flags, VkExternalMemoryHandleTypeFlagBits handle_type,
    VkExternalMemoryProperties* external_memory_properties) 
{
  ...
  VkResult result = vkGetPhysicalDeviceImageFormatProperties2(physical_device, &format_info_2, &image_format_properties_2);
  if (result != VK_SUCCESS) 
  {
    return result;
  }

  *external_memory_properties = external_image_format_properties.externalMemoryProperties;
  return VK_SUCCESS;
}

std::vector<VkDrmFormatModifierPropertiesEXT>
QueryVkDrmFormatModifierPropertiesEXT(VkPhysicalDevice physical_device, VkFormat format) 
{
...
{
  UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("GPU.Vulkan.SkiaContext.vkCreateGraphicsPipelinesUS", sample,
...
void EmitVkQueueSubmitUMA(base::TimeDelta sample) 
{
  ...
  {
    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("GPU.Vulkan.SkiaContext.vkQueueSubmitUS", sample,
        base::Microseconds(50), base::Microseconds(1'000'000), 50);
  }
}
