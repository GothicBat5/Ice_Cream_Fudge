...
bool VulkanInstance::CreateInstance(
...
#if DCHECK_IS_ON()
  for (const char* enabled_extension : vulkan_info_.enabled_instance_extensions) 
  {
    bool found = false;
    for (const VkExtensionProperties& ext_property :
         vulkan_info_.instance_extensions) {
      if (UNSAFE_TODO(strcmp(ext_property.extensionName, enabled_extension)) == 0) 
      {
        found = true;
        break;
      }
  ...
  VkInstanceCreateInfo instance_create_info = {
      ...
      base::checked_cast<uint32_t>(vulkan_info_.enabled_instance_extensions.size()),
      // enabledExtensionCount
      vulkan_info_.enabled_instance_extensions.data(),
      // ppEnabledExtensionNames
  };

  VkResult result = vkCreateInstance(&instance_create_info, nullptr, &owned_vk_instance_);
  ...
  gfx::ExtensionSet enabled_extensions(std::begin(vulkan_info_.enabled_instance_extensions),
    ...
    const std::vector<const char*>& required_layers) 
{
  ...
  VulkanFunctionPointers* vulkan_function_pointers =
      gpu::GetVulkanFunctionPointers();
  if (!vulkan_function_pointers->BindInstanceFunctionPointers(vk_instance_, vulkan_info_.used_api_version, extensions)) 
  {
    return false;
  }
    ...
    const std::vector<const char*>& required_layers) 
{
  ...
  for (const char* layer_name : all_required_layers) 
  {
    ...
    const size_t previous_extension_count = vulkan_info_.instance_extensions.size();
    vulkan_info_.instance_extensions.resize(previous_extension_count + num_instance_exts);
    result = vkEnumerateInstanceExtensionProperties(layer_name, &num_instance_exts,
        UNSAFE_TODO(&vulkan_info_.instance_extensions.data()[previous_extension_count]));
  ...
  for (const VkExtensionProperties& ext_property :
       vulkan_info_.instance_extensions) {
    if (UNSAFE_TODO(strcmp(ext_property.extensionName, VK_EXT_DEBUG_REPORT_EXTENSION_NAME)) == 0) 
    {
      debug_report_enabled_ = true;
      vulkan_info_.enabled_instance_extensions.push_back(VK_EXT_DEBUG_REPORT_EXTENSION_NAME);
    }
  }
  ...
  result = vkEnumerateInstanceLayerProperties(&num_instance_layers, vulkan_info_.instance_layers.data());
...
bool VulkanInstance::CollectDeviceInfo(VkPhysicalDevice physical_device) 
{
  ...
  if (physical_device == VK_NULL_HANDLE) 
  {
    ...
    result = vkEnumeratePhysicalDevices(vk_instance_, &count, physical_devices.data());
  ...
  for (VkPhysicalDevice device : physical_devices) 
  {
    ...
    uint32_t count = 0;
    VkResult result = vkEnumerateDeviceExtensionProperties(device, nullptr /* pLayerName */, &count, nullptr);
    LOG_IF(ERROR, result != VK_SUCCESS)
        << "vkEnumerateDeviceExtensionProperties failed: " << result;

    info.extensions.resize(count);
    result = vkEnumerateDeviceExtensionProperties(device, nullptr /* pLayerName */, &count, info.extensions.data());
    LOG_IF(ERROR, result != VK_SUCCESS)
        << "vkEnumerateDeviceExtensionProperties failed: " << result;
    ...
    {
      bool has_drm_extension = std::ranges::any_of(info.extensions, [](const auto& ext) 
      {
      ...
      {
        static_assert(sizeof(dev_t) <= sizeof(info.drm_device_id), "unexpected dev_t size");
        if (drm_properties.hasRender) 
        {
          info.drm_device_id = makedev(drm_properties.renderMajor, drm_properties.renderMinor);
        } 
        else {
          info.drm_device_id =
      ...
      VkPhysicalDeviceSamplerYcbcrConversionFeatures ycbcr_conversion_features = {VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SAMPLER_YCBCR_CONVERSION_FEATURES};
    ...
    if (count) 
    {
      ...
      vkGetPhysicalDeviceQueueFamilyProperties(device, &count, info.queue_families.data());
    }
  }
  return true;
}
